...
def parse_args():
    ...
    args.modelDir = '--writeBoxFrames/models/pytorch/pose_coco/pose_hrnet_w32_256x256.pth'
    args.logDir = '--videoFile/data/videos/video.mp4'
    args.dataDir = ''
    args.prevModelDir = ''
    return args
...
def main():
    ...
    # Loading an video
    # 创建VideoCapture捕获对象，cv2.VideoCapture('filename')
    vidcap = cv2.VideoCapture(args.videoFile)
    # 得到视频的帧率
    fps = vidcap.get(cv2.CAP_PROP_FPS)
    # 要求视频帧率必须大于我们给定的inferenceFps的帧率
    ...
    # 指定输出路径，即输出目录
    # cv2.VideoWriter(1， 2， 3， 4),1保存路径，2是编码器，
    # 3是帧率，4保存文件的画面尺寸
    # cv2.VideoWriter_fourcc('MJPG') ---即格式是avi
    # cv2.VideoWriter_fourcc('mp4v') ---即格式是MP4格式
    ...
    while vidcap.isOpened():
        # 记录当前系统时间
        total_now = time.time()
        # 读取视频的每一张图像，read()返回ret和图像，其中ret为true
        ret, image_bgr = vidcap.read()
        # 记录次数，记录输出图像的个数
        count += 1
        # 如果读取完视频所有的帧了，那么ret=false，
        # 则直接跳出本次循环
        if not ret:
            continue
        # 间隔采样
